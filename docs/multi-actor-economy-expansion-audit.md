...
**Hub six implemented 2026-08-01:** Kiln Crossing is the second industrial producer and the sixth settlement generated from the common catalog. Ansa Vale begins with a larger silicate shelf and treasury than The Ledge but operates from a longer, newly authored trace beyond it, with a cautious low-growth policy and a different household economy. Supplier diagnostics expose both legal industrial candidates. Kiln wins opening work when its ready stock offsets the longer route; The Ledge wins the same comparison when Kiln's finite capacity is committed. This is the first focused proof that the winner can change because of live operating state rather than catalog order, and no shared economic system names either result. The six-hub topology now exists; long-run switching, pillar-removal recovery, and competitive repair remain the principal unfinished proofs.
...
**Paperwork and wreck-title foundation implemented 2026-08-01:** Commercial craft now publish one inspection shape containing VIN, operator and operating license, registration, title, owner institution, transponder, authorized activities, and known hubs. Seeded and newly commissioned haulers use it, as do every Cinder and Flint mining craft. Existing hub and patrol scanning validates the records and retains the legal details in the registry and ledger; recognizing a familiar hull is no longer sufficient clearance. Destruction preserves the active ship title as an owner-controlled wreck record instead of creating ownerless salvage. The wreck remains a physical drifting object that the player's existing tow cable can hook and drag. A separate salvage-authorization transition names the salvor and destination, providing the legal seam for the next contract slice; finding or towing a wreck alone does not transfer its title.
