...
- A corridor may also define an authored course profile over the procedural centerline. The first freight corridor follows several natural bends, a broad detour, and one modest backtracking hook with small seeded variation and arc-length-spaced navigation points. Irregular, destructible inner and outer rock shoulders define most of the road visually; intentional gaps make dangerous shoot-through shortcuts possible. NPC haulers reduce speed according to upcoming turn angle, while ordinary corridor travel receives the lower wear expected from an engineered road.
- The corridor provides a 20% player maximum-speed slipstream and a 10% thrust improvement. Four visible boost patches instantly double the ship's current velocity. The resulting kinetic overspeed is permitted above the normal limiter and burns away through ordinary flight drag rather than a timed engine modifier. A low continuous perpendicular maintenance pressure nudges debris outward and slowly relocates its spring anchor beyond the road edge, allowing naturally displaced and encounter-generated rocks to clear without being launched.
...
The first upstream production proof now precedes that chain. Each active hub publishes one evergreen local extraction order. Cinder Contracting, controlled by licensed miner Ivo Cinder, owns three registered worker ships and can approve a fourth after sustained funded pressure. Workers use shared flight physics, regional deposit knowledge, remote asteroid streaming, physical mining shots, and tractor fields. They recover their own mined pickups plus loose unclaimed resources, but do not treat another claim's loose cargo as free salvage. Critical SPRC orders outrank ordinary extraction; miners can collect up to six requested units across multiple rocks before returning. Remainder-run surplus is sold into the local buyer's real inventory. Rejected deliveries preserve cargo and assignment. Delivered materials enter the same hub inventory used by player contracts and freight containers. Empty freight sources no longer synthesize replacement stock; haulers wait for real inventory.
