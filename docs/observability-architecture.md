...
- **miningOperation** — committed state + decision with alternatives and reasons; idle blocker (`no-eligible-work`); **`order-fully-allocated`** and **`no-route-to-destination`** blockers on paths that previously returned silently; **`outbid`** when a ship lost the shared extraction clearing, carrying the order, its own bid, and the ship and controller that beat it (`causedBy` points at the winner, so the chain walks into a competitor); `awaiting-service` on breakdown (pointing at SPRC); `unpaid-service-debt`; cleared to `free` on paid service. Plus a **fleet-level institution diagnostic** (`all-suppliers-committed` / `no-eligible-work`) so chains terminate in real reasoning.
  - `outbid` and `no-eligible-work` are deliberately different facts. Reporting a lost auction as "nothing worth taking" is how an unfair allocation rule stayed invisible: a company that never won looked like one that never wanted to.
- **patrolOperations / protectionProviders** — every response craft, hub watch and contracted alike, now records its own diagnostic: `free` on station, `working` while deployed or returning, `disabled` as a wreck with the replacement cost it cannot yet fund in `waitingFor`. Before this, a settlement's watch craft was invisible to the observatory and a request could read "covered" while the craft it named sat at the dock — the projection agreed with the plan instead of with the world.
