...
The authority foundation now has a shared place registry and rule checker. Regions are seeded as `Place` records at game start, and region rights from `worldRegions.js` are seeded as authority grants. Claims/plots should follow the same path next: they should become queryable `Place` records, not only panorama overlay geometry. That is what will let contracts, patrols, mining rights, fly-through corridors, and future ownership checks ask the same question: who can do what here?
