...
16. Rook evaluates the delivery and sends the player to buy a miner-equipped ship.
...
Contracts are now a separate layer from missions. Rook's first job offers the `Assessment Delivery` contract. Docking the attached VIN at Yard Exchange fulfills the terms, then the player completes the contract card to receive 500 credits. The mission does not directly award money; it waits for the contract system to record `contract.paid`. That keeps story pacing separate from enforceable terms, which is the same foundation future loans, deadlines, damage penalties, bonuses, and hub job boards can use.
...
After buying the starter mining ship, Rook returns with the first resource-work loop. `First Red Run` offers a repeatable contract for 5 red resources delivered to Yard Exchange at 100 credits per unit. Resource delivery is contract-driven rather than mission-driven: depositing the required red units fulfills the contract, then the player completes the contract card to receive payment. The mission listens for that paid event and handles the story wrap-up.
