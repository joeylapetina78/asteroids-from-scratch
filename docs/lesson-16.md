...
18. Accepting the loan lets the player buy the Rook special ship package with a miner, processor, and cargo hold.
...
The second mission introduces the first shipyard slice. Ship offers live in data, and the Merchant panel renders them into cards. The Rook special is the only realistic starter purchase; the other offers show the wider brand/model ladder before the player can afford it. The loan contract is intentionally simple for now: it deposits credits and records debt, while actual repayment UI is left for a later pass.
