import { Bullet } from "./entities/Bullet.js?v=fresh-20260916-2047-f04aec86";
import { steerAroundObstacles } from "./systems/obstacleNavigation.js?v=fresh-20260916-2047-f04aec86";
import { getHullOutline } from "./content/ships/hullOutlines.js?v=fresh-20260916-2047-f04aec86";
import { getBuildProgress } from "./systems/shipyards.js?v=fresh-20260916-2047-f04aec86";
import { facilityOffset } from "./systems/hubLayout.js?v=fresh-20260916-2047-f04aec86";
import { VIEWPORT_SCOPE_MARGIN } from "./systems/cockpitSnap.js?v=fresh-20260916-2047-f04aec86";
import { infrastructureNavigationObstacles } from "./systems/infrastructureNavigation.js?v=fresh-20260916-2047-f04aec86";
import { breakAsteroid, WHITE_ASTEROID_COLOR } from "./entities/Asteroid.js?v=fresh-20260916-2047-f04aec86";
import { createResourcePickupsFromAsteroid, ResourcePickup } from "./entities/ResourcePickup.js?v=fresh-20260916-2047-f04aec86";
import { drawResourceShape } from "./entities/ResourcePickup.js?v=fresh-20260916-2047-f04aec86";
import { Ship } from "./entities/Ship.js?v=fresh-20260916-2047-f04aec86";
import { ShipWreck } from "./entities/ShipWreck.js?v=fresh-20260916-2047-f04aec86";
import { completeWreckSalvage, registerOwnedWreck } from "./systems/wreckRegistry.js?v=fresh-20260916-2047-f04aec86";
import { createAsteroidChunks, getOreClusterSeedsInRadius } from "./systems/asteroidField.js?v=fresh-20260916-2047-f04aec86";
import { applyPanelPatch, getHullRepairRateMultiplier, HULL_REPAIR_DELAY_SECONDS, HULL_REPAIR_RATE, accumulatePanelWear, ensurePanelCondition, panelStageIndex, repairPanelCondition } from "./systems/panelMaintenance.js?v=fresh-20260916-2047-f04aec86";
import { ENGINE_CONDITION_CONFIG, computeEngineWearDelta, getEngineStageEffects } from "./systems/engineCondition.js?v=fresh-20260916-2047-f04aec86";
import { MINER_CONDITION_CONFIG, computeMinerWearPerShot, getMinerStageEffects } from "./systems/minerCondition.js?v=fresh-20260916-2047-f04aec86";
import { COLLECTOR_CONDITION_CONFIG, computeCollectorWearPerSecond, getCollectorStageEffects } from "./systems/collectorCondition.js?v=fresh-20260916-2047-f04aec86";
import { createCamera } from "./systems/camera.js?v=fresh-20260916-2047-f04aec86";
import { createInput } from "./systems/input.js?v=fresh-20260916-2047-f04aec86";
import { GREATBLOOM_EYE_RADIUS } from "./entities/Lifeform.js?v=fresh-20260916-2047-f04aec86";
import { BLOOM_HARVEST_RESOURCE, BLOOM_KILLS_PER_GREATBLOOM, createAmbientLifeBatch, createGrazerAtFeast, createHunterNearShip, createHunterRespawn, createLifeField, createSurfacingGreatbloom, rollBloomHarvest, seedChunkRockmoss } from "./systems/lifeField.js?v=fresh-20260916-2047-f04aec86";
import { ROCKMOSS_CRAWLER_TYPE, ROCKMOSS_STRAINS, pickRockmossStrain } from "./systems/rockmossStrains.js?v=fresh-20260916-2047-f04aec86";
import { GRAZING_DEFAULTS, advanceGrazing, condenseDistantApexGrazers, condenseGrazerPopulation, enforceEcologicalBiomassBudget, findGrazingClusters, getGrazerSporeYield, isRipe } from "./systems/grazing.js?v=fresh-20260916-2047-f04aec86";
import { createRandom, hashNumbers } from "./systems/random.js?v=fresh-20260916-2047-f04aec86";
import { HAULER_PALETTES, RELIEF_HAULER_PALETTE, createNpcRouteShips, createRouteShip } from "./systems/npcRoutes.js?v=fresh-20260916-2047-f04aec86";
import { clearScreen, drawGrid, drawVector, isVisible } from "./systems/rendering.js?v=fresh-20260916-2047-f04aec86";
import { createResourceField } from "./systems/resourceField.js?v=fresh-20260916-2047-f04aec86";
import { createScanner } from "./systems/scanner.js?v=fresh-20260916-2047-f04aec86";
import { createDriftMouthField } from "./systems/driftMouthField.js?v=fresh-20260916-2047-f04aec86";
import { createIncursionField, createSeededDevice } from "./systems/incursionField.js?v=fresh-20260916-2047-f04aec86";
import { completeInternalProtectionResponse, ensurePatrolOperations, failInternalProtectionResponse, finishInternalProtectionReturn, getAvailablePatrolCraft, markPatrolCraftStatus, servicePatrolCraft, startInternalProtectionResponse } from "./systems/patrolOperations.js?v=fresh-20260916-2047-f04aec86";
import { createPatrolRuntimeActor } from "./systems/patrolRuntime.js?v=fresh-20260916-2047-f04aec86";
import { listPendingPatrolResponses } from "./systems/patrolDispatch.js?v=fresh-20260916-2047-f04aec86";
import { closeProtectionRequestsForThreat, evaluateProtectionThreat, getPlayerProtectionJobsForSite, reviewProtectionRequests } from "./systems/protectionPlanning.js?v=fresh-20260916-2047-f04aec86";
import { completePlayerProtectionRequest, completeProtectionContract, ensureProtectionProviders, failProtectionContract, finishProtectionReturn, serviceProtectionProviders, startProtectionContract } from "./systems/protectionProviders.js?v=fresh-20260916-2047-f04aec86";
import { fileAttackReport, nearestActiveReport, resolveAttackReport } from "./systems/securityReports.js?v=fresh-20260916-2047-f04aec86";
import { injectBountyJobs } from "./systems/bountyContracts.js?v=fresh-20260916-2047-f04aec86";
import { injectCargoRuns } from "./systems/cargoContracts.js?v=fresh-20260916-2047-f04aec86";
import { getStandingFreightJobsForSite } from "./systems/logistics.js?v=fresh-20260916-2047-f04aec86";
import { getRuntimeWorldConnections, getRuntimeWorldSites } from "./systems/worldNetworkRegistry.js?v=fresh-20260916-2047-f04aec86";
import { applyCorridorMaintenance, createTransportCorridors, getCorridorClearance } from "./systems/transportCorridors.js?v=fresh-20260916-2047-f04aec86";
import { getStandingMiningJobsForSite } from "./systems/miningOperation.js?v=fresh-20260916-2047-f04aec86";
import { generateSurveyContractDefinition, generateSurveyJobBoardDefinitions } from "./systems/surveyContracts.js?v=fresh-20260916-2047-f04aec86";
import { createEncounterDirector } from "./systems/encounterDirector.js?v=fresh-20260916-2047-f04aec86";
import { createPortalTrophy, getHostileLootCount, rollHostileLoot } from "./systems/hostileLoot.js?v=fresh-20260916-2047-f04aec86";
import { awardGateBountyToPatrol } from "./systems/gateBounty.js?v=fresh-20260916-2047-f04aec86";
import { createThreadwyrmField } from "./systems/threadwyrmField.js?v=fresh-20260916-2047-f04aec86";
import { recordVisitedZone } from "./systems/legalRecords.js?v=fresh-20260916-2047-f04aec86";
import { getSectorDesignation } from "./systems/sectorCodes.js?v=fresh-20260916-2047-f04aec86";
import { sampleEnvironment, getFlowAngle } from "./systems/worldHazards.js?v=fresh-20260916-2047-f04aec86";
import { inspectPublicIdentity } from "./systems/authorityInspections.js?v=fresh-20260916-2047-f04aec86";
import { getRegistryEntityIdForSite, getRegistrySubject, hasRegistryStatus, rememberRegistrySubject } from "./systems/entityRegistry.js?v=fresh-20260916-2047-f04aec86";
import { createCommercialCraftPublicIdentity, createControlledShipPublicIdentity, createNpcShipPublicIdentity } from "./systems/publicIdentity.js?v=fresh-20260916-2047-f04aec86";
import { getZoneProfile, WORLD_ZONES, getZoneInfluence } from "./systems/worldZones.js?v=fresh-20260916-2047-f04aec86";
import { getRegionProfile } from "./systems/worldRegions.js?v=fresh-20260916-2047-f04aec86";
import { createClaimField } from "./systems/claimField.js?v=fresh-20260916-2047-f04aec86";
import { getContractGrantedClaimIds, getPlotRestriction } from "./systems/operatingRights.js?v=fresh-20260916-2047-f04aec86";
import { getNearbyWorldSite, getNearestWorldSite, isInSiteRange } from "./systems/worldSites.js?v=fresh-20260916-2047-f04aec86";
import { createGameState } from "./state/gameState.js?v=fresh-20260916-2047-f04aec86";
import { canSpendCredits, debitCredits, depositCredits, getCredits, spendCredits } from "./systems/accounts.js?v=fresh-20260916-2047-f04aec86";
import { PLAYER_ATTRIBUTED_CAUSES } from "./systems/eventLedger.js?v=fresh-20260916-2047-f04aec86";
import { getResourceColor, getResourceShape, resourceTypesMatch } from "./systems/resourceDefinitions.js?v=fresh-20260916-2047-f04aec86";
import { terminateDestroyedActor } from "./systems/actorLifecycle.js?v=fresh-20260916-2047-f04aec86";
