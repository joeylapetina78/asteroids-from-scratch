import { Bullet } from "./entities/Bullet.js?v=fresh-20260918-2012-165f50cd";
import { steerAroundObstacles } from "./systems/obstacleNavigation.js?v=fresh-20260918-2012-165f50cd";
import { getHullOutline } from "./content/ships/hullOutlines.js?v=fresh-20260918-2012-165f50cd";
import { getBuildProgress } from "./systems/shipyards.js?v=fresh-20260918-2012-165f50cd";
import { facilityOffset } from "./systems/hubLayout.js?v=fresh-20260918-2012-165f50cd";
import { VIEWPORT_SCOPE_MARGIN } from "./systems/cockpitSnap.js?v=fresh-20260918-2012-165f50cd";
import { infrastructureNavigationObstacles } from "./systems/infrastructureNavigation.js?v=fresh-20260918-2012-165f50cd";
import { breakAsteroid, WHITE_ASTEROID_COLOR } from "./entities/Asteroid.js?v=fresh-20260918-2012-165f50cd";
import { createResourcePickupsFromAsteroid, ResourcePickup } from "./entities/ResourcePickup.js?v=fresh-20260918-2012-165f50cd";
import { drawResourceShape } from "./entities/ResourcePickup.js?v=fresh-20260918-2012-165f50cd";
import { Ship } from "./entities/Ship.js?v=fresh-20260918-2012-165f50cd";
import { ShipWreck } from "./entities/ShipWreck.js?v=fresh-20260918-2012-165f50cd";
import { completeWreckSalvage, registerOwnedWreck } from "./systems/wreckRegistry.js?v=fresh-20260918-2012-165f50cd";
import { createAsteroidChunks, getOreClusterSeedsInRadius } from "./systems/asteroidField.js?v=fresh-20260918-2012-165f50cd";
import { applyPanelPatch, getHullRepairRateMultiplier, HULL_REPAIR_DELAY_SECONDS, HULL_REPAIR_RATE, accumulatePanelWear, ensurePanelCondition, panelStageIndex, repairPanelCondition } from "./systems/panelMaintenance.js?v=fresh-20260918-2012-165f50cd";
import { ENGINE_CONDITION_CONFIG, computeEngineWearDelta, getEngineStageEffects } from "./systems/engineCondition.js?v=fresh-20260918-2012-165f50cd";
import { MINER_CONDITION_CONFIG, computeMinerWearPerShot, getMinerStageEffects } from "./systems/minerCondition.js?v=fresh-20260918-2012-165f50cd";
import { COLLECTOR_CONDITION_CONFIG, computeCollectorWearPerSecond, getCollectorStageEffects } from "./systems/collectorCondition.js?v=fresh-20260918-2012-165f50cd";
import { createCamera } from "./systems/camera.js?v=fresh-20260918-2012-165f50cd";
import { createInput } from "./systems/input.js?v=fresh-20260918-2012-165f50cd";
import { GREATBLOOM_EYE_RADIUS } from "./entities/Lifeform.js?v=fresh-20260918-2012-165f50cd";
import { BLOOM_HARVEST_RESOURCE, BLOOM_KILLS_PER_GREATBLOOM, createAmbientLifeBatch, createGrazerAtFeast, createHunterNearShip, createHunterRespawn, createLifeField, createSurfacingGreatbloom, rollBloomHarvest, seedChunkRockmoss } from "./systems/lifeField.js?v=fresh-20260918-2012-165f50cd";
import { ROCKMOSS_CRAWLER_TYPE, ROCKMOSS_STRAINS, pickRockmossStrain } from "./systems/rockmossStrains.js?v=fresh-20260918-2012-165f50cd";
import { GRAZING_DEFAULTS, advanceGrazing, condenseDistantApexGrazers, condenseGrazerPopulation, enforceEcologicalBiomassBudget, findGrazingClusters, getGrazerSporeYield, isRipe } from "./systems/grazing.js?v=fresh-20260918-2012-165f50cd";
import { createRandom, hashNumbers } from "./systems/random.js?v=fresh-20260918-2012-165f50cd";
import { HAULER_PALETTES, RELIEF_HAULER_PALETTE, createNpcRouteShips, createRouteShip } from "./systems/npcRoutes.js?v=fresh-20260918-2012-165f50cd";
import { clearScreen, drawGrid, drawVector, isVisible } from "./systems/rendering.js?v=fresh-20260918-2012-165f50cd";
import { createResourceField } from "./systems/resourceField.js?v=fresh-20260918-2012-165f50cd";
import { createScanner } from "./systems/scanner.js?v=fresh-20260918-2012-165f50cd";
import { createDriftMouthField } from "./systems/driftMouthField.js?v=fresh-20260918-2012-165f50cd";
import { createIncursionField, createSeededDevice } from "./systems/incursionField.js?v=fresh-20260918-2012-165f50cd";
import { completeInternalProtectionResponse, ensurePatrolOperations, failInternalProtectionResponse, finishInternalProtectionReturn, getAvailablePatrolCraft, markPatrolCraftStatus, servicePatrolCraft, startInternalProtectionResponse } from "./systems/patrolOperations.js?v=fresh-20260918-2012-165f50cd";
import { createPatrolRuntimeActor } from "./systems/patrolRuntime.js?v=fresh-20260918-2012-165f50cd";
import { listPendingPatrolResponses } from "./systems/patrolDispatch.js?v=fresh-20260918-2012-165f50cd";
import { closeProtectionRequestsForThreat, evaluateProtectionThreat, getPlayerProtectionJobsForSite, reviewProtectionRequests } from "./systems/protectionPlanning.js?v=fresh-20260918-2012-165f50cd";
import { completePlayerProtectionRequest, completeProtectionContract, ensureProtectionProviders, failProtectionContract, finishProtectionReturn, serviceProtectionProviders, startProtectionContract } from "./systems/protectionProviders.js?v=fresh-20260918-2012-165f50cd";
import { fileAttackReport, nearestActiveReport, resolveAttackReport } from "./systems/securityReports.js?v=fresh-20260918-2012-165f50cd";
import { injectBountyJobs } from "./systems/bountyContracts.js?v=fresh-20260918-2012-165f50cd";
import { injectCargoRuns } from "./systems/cargoContracts.js?v=fresh-20260918-2012-165f50cd";
import { getStandingFreightJobsForSite } from "./systems/logistics.js?v=fresh-20260918-2012-165f50cd";
import { getRuntimeWorldConnections, getRuntimeWorldSites } from "./systems/worldNetworkRegistry.js?v=fresh-20260918-2012-165f50cd";
import { applyCorridorMaintenance, createTransportCorridors, getCorridorClearance } from "./systems/transportCorridors.js?v=fresh-20260918-2012-165f50cd";
import { getStandingMiningJobsForSite } from "./systems/miningOperation.js?v=fresh-20260918-2012-165f50cd";
import { generateSurveyContractDefinition, generateSurveyJobBoardDefinitions } from "./systems/surveyContracts.js?v=fresh-20260918-2012-165f50cd";
import { createEncounterDirector } from "./systems/encounterDirector.js?v=fresh-20260918-2012-165f50cd";
import { createPortalTrophy, getHostileLootCount, rollHostileLoot } from "./systems/hostileLoot.js?v=fresh-20260918-2012-165f50cd";
import { awardGateBountyToPatrol } from "./systems/gateBounty.js?v=fresh-20260918-2012-165f50cd";
import { createThreadwyrmField } from "./systems/threadwyrmField.js?v=fresh-20260918-2012-165f50cd";
import { recordVisitedZone } from "./systems/legalRecords.js?v=fresh-20260918-2012-165f50cd";
import { getSectorDesignation } from "./systems/sectorCodes.js?v=fresh-20260918-2012-165f50cd";
import { sampleEnvironment, getFlowAngle } from "./systems/worldHazards.js?v=fresh-20260918-2012-165f50cd";
import { inspectPublicIdentity } from "./systems/authorityInspections.js?v=fresh-20260918-2012-165f50cd";
import { getRegistryEntityIdForSite, getRegistrySubject, hasRegistryStatus, rememberRegistrySubject } from "./systems/entityRegistry.js?v=fresh-20260918-2012-165f50cd";
import { createCommercialCraftPublicIdentity, createControlledShipPublicIdentity, createNpcShipPublicIdentity } from "./systems/publicIdentity.js?v=fresh-20260918-2012-165f50cd";
import { getZoneProfile, WORLD_ZONES, getZoneInfluence } from "./systems/worldZones.js?v=fresh-20260918-2012-165f50cd";
import { getRegionProfile } from "./systems/worldRegions.js?v=fresh-20260918-2012-165f50cd";
import { createClaimField } from "./systems/claimField.js?v=fresh-20260918-2012-165f50cd";
import { getContractGrantedClaimIds, getPlotRestriction } from "./systems/operatingRights.js?v=fresh-20260918-2012-165f50cd";
import { getNearbyWorldSite, getNearestWorldSite, isInSiteRange } from "./systems/worldSites.js?v=fresh-20260918-2012-165f50cd";
import { createGameState } from "./state/gameState.js?v=fresh-20260918-2012-165f50cd";
import { canSpendCredits, debitCredits, depositCredits, getCredits, spendCredits } from "./systems/accounts.js?v=fresh-20260918-2012-165f50cd";
import { PLAYER_ATTRIBUTED_CAUSES } from "./systems/eventLedger.js?v=fresh-20260918-2012-165f50cd";
import { getResourceColor, getResourceShape, resourceTypesMatch } from "./systems/resourceDefinitions.js?v=fresh-20260918-2012-165f50cd";
import { terminateDestroyedActor } from "./systems/actorLifecycle.js?v=fresh-20260918-2012-165f50cd";
