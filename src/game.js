import { Bullet } from "./entities/Bullet.js?v=fresh-20260918-1918-133a739f";
import { steerAroundObstacles } from "./systems/obstacleNavigation.js?v=fresh-20260918-1918-133a739f";
import { getHullOutline } from "./content/ships/hullOutlines.js?v=fresh-20260918-1918-133a739f";
import { getBuildProgress } from "./systems/shipyards.js?v=fresh-20260918-1918-133a739f";
import { facilityOffset } from "./systems/hubLayout.js?v=fresh-20260918-1918-133a739f";
import { VIEWPORT_SCOPE_MARGIN } from "./systems/cockpitSnap.js?v=fresh-20260918-1918-133a739f";
import { infrastructureNavigationObstacles } from "./systems/infrastructureNavigation.js?v=fresh-20260918-1918-133a739f";
import { breakAsteroid, WHITE_ASTEROID_COLOR } from "./entities/Asteroid.js?v=fresh-20260918-1918-133a739f";
import { createResourcePickupsFromAsteroid, ResourcePickup } from "./entities/ResourcePickup.js?v=fresh-20260918-1918-133a739f";
import { drawResourceShape } from "./entities/ResourcePickup.js?v=fresh-20260918-1918-133a739f";
import { Ship } from "./entities/Ship.js?v=fresh-20260918-1918-133a739f";
import { ShipWreck } from "./entities/ShipWreck.js?v=fresh-20260918-1918-133a739f";
import { completeWreckSalvage, registerOwnedWreck } from "./systems/wreckRegistry.js?v=fresh-20260918-1918-133a739f";
import { createAsteroidChunks, getOreClusterSeedsInRadius } from "./systems/asteroidField.js?v=fresh-20260918-1918-133a739f";
import { applyPanelPatch, getHullRepairRateMultiplier, HULL_REPAIR_DELAY_SECONDS, HULL_REPAIR_RATE, accumulatePanelWear, ensurePanelCondition, panelStageIndex, repairPanelCondition } from "./systems/panelMaintenance.js?v=fresh-20260918-1918-133a739f";
import { ENGINE_CONDITION_CONFIG, computeEngineWearDelta, getEngineStageEffects } from "./systems/engineCondition.js?v=fresh-20260918-1918-133a739f";
import { MINER_CONDITION_CONFIG, computeMinerWearPerShot, getMinerStageEffects } from "./systems/minerCondition.js?v=fresh-20260918-1918-133a739f";
import { COLLECTOR_CONDITION_CONFIG, computeCollectorWearPerSecond, getCollectorStageEffects } from "./systems/collectorCondition.js?v=fresh-20260918-1918-133a739f";
import { createCamera } from "./systems/camera.js?v=fresh-20260918-1918-133a739f";
import { createInput } from "./systems/input.js?v=fresh-20260918-1918-133a739f";
import { GREATBLOOM_EYE_RADIUS } from "./entities/Lifeform.js?v=fresh-20260918-1918-133a739f";
import { BLOOM_HARVEST_RESOURCE, BLOOM_KILLS_PER_GREATBLOOM, createAmbientLifeBatch, createGrazerAtFeast, createHunterNearShip, createHunterRespawn, createLifeField, createSurfacingGreatbloom, rollBloomHarvest, seedChunkRockmoss } from "./systems/lifeField.js?v=fresh-20260918-1918-133a739f";
import { ROCKMOSS_CRAWLER_TYPE, ROCKMOSS_STRAINS, pickRockmossStrain } from "./systems/rockmossStrains.js?v=fresh-20260918-1918-133a739f";
import { GRAZING_DEFAULTS, advanceGrazing, condenseDistantApexGrazers, condenseGrazerPopulation, enforceEcologicalBiomassBudget, findGrazingClusters, getGrazerSporeYield, isRipe } from "./systems/grazing.js?v=fresh-20260918-1918-133a739f";
import { createRandom, hashNumbers } from "./systems/random.js?v=fresh-20260918-1918-133a739f";
import { HAULER_PALETTES, RELIEF_HAULER_PALETTE, createNpcRouteShips, createRouteShip } from "./systems/npcRoutes.js?v=fresh-20260918-1918-133a739f";
import { clearScreen, drawGrid, drawVector, isVisible } from "./systems/rendering.js?v=fresh-20260918-1918-133a739f";
import { createResourceField } from "./systems/resourceField.js?v=fresh-20260918-1918-133a739f";
import { createScanner } from "./systems/scanner.js?v=fresh-20260918-1918-133a739f";
import { createDriftMouthField } from "./systems/driftMouthField.js?v=fresh-20260918-1918-133a739f";
import { createIncursionField, createSeededDevice } from "./systems/incursionField.js?v=fresh-20260918-1918-133a739f";
import { completeInternalProtectionResponse, ensurePatrolOperations, failInternalProtectionResponse, finishInternalProtectionReturn, getAvailablePatrolCraft, markPatrolCraftStatus, servicePatrolCraft, startInternalProtectionResponse } from "./systems/patrolOperations.js?v=fresh-20260918-1918-133a739f";
import { createPatrolRuntimeActor } from "./systems/patrolRuntime.js?v=fresh-20260918-1918-133a739f";
import { listPendingPatrolResponses } from "./systems/patrolDispatch.js?v=fresh-20260918-1918-133a739f";
import { closeProtectionRequestsForThreat, evaluateProtectionThreat, getPlayerProtectionJobsForSite, reviewProtectionRequests } from "./systems/protectionPlanning.js?v=fresh-20260918-1918-133a739f";
import { completePlayerProtectionRequest, completeProtectionContract, ensureProtectionProviders, failProtectionContract, finishProtectionReturn, serviceProtectionProviders, startProtectionContract } from "./systems/protectionProviders.js?v=fresh-20260918-1918-133a739f";
import { fileAttackReport, nearestActiveReport, resolveAttackReport } from "./systems/securityReports.js?v=fresh-20260918-1918-133a739f";
import { injectBountyJobs } from "./systems/bountyContracts.js?v=fresh-20260918-1918-133a739f";
import { injectCargoRuns } from "./systems/cargoContracts.js?v=fresh-20260918-1918-133a739f";
import { getStandingFreightJobsForSite } from "./systems/logistics.js?v=fresh-20260918-1918-133a739f";
import { getRuntimeWorldConnections, getRuntimeWorldSites } from "./systems/worldNetworkRegistry.js?v=fresh-20260918-1918-133a739f";
import { applyCorridorMaintenance, createTransportCorridors, getCorridorClearance } from "./systems/transportCorridors.js?v=fresh-20260918-1918-133a739f";
import { getStandingMiningJobsForSite } from "./systems/miningOperation.js?v=fresh-20260918-1918-133a739f";
import { generateSurveyContractDefinition, generateSurveyJobBoardDefinitions } from "./systems/surveyContracts.js?v=fresh-20260918-1918-133a739f";
import { createEncounterDirector } from "./systems/encounterDirector.js?v=fresh-20260918-1918-133a739f";
import { createPortalTrophy, getHostileLootCount, rollHostileLoot } from "./systems/hostileLoot.js?v=fresh-20260918-1918-133a739f";
import { awardGateBountyToPatrol } from "./systems/gateBounty.js?v=fresh-20260918-1918-133a739f";
import { createThreadwyrmField } from "./systems/threadwyrmField.js?v=fresh-20260918-1918-133a739f";
import { recordVisitedZone } from "./systems/legalRecords.js?v=fresh-20260918-1918-133a739f";
import { getSectorDesignation } from "./systems/sectorCodes.js?v=fresh-20260918-1918-133a739f";
import { sampleEnvironment, getFlowAngle } from "./systems/worldHazards.js?v=fresh-20260918-1918-133a739f";
import { inspectPublicIdentity } from "./systems/authorityInspections.js?v=fresh-20260918-1918-133a739f";
import { getRegistryEntityIdForSite, getRegistrySubject, hasRegistryStatus, rememberRegistrySubject } from "./systems/entityRegistry.js?v=fresh-20260918-1918-133a739f";
import { createCommercialCraftPublicIdentity, createControlledShipPublicIdentity, createNpcShipPublicIdentity } from "./systems/publicIdentity.js?v=fresh-20260918-1918-133a739f";
import { getZoneProfile, WORLD_ZONES, getZoneInfluence } from "./systems/worldZones.js?v=fresh-20260918-1918-133a739f";
import { getRegionProfile } from "./systems/worldRegions.js?v=fresh-20260918-1918-133a739f";
import { createClaimField } from "./systems/claimField.js?v=fresh-20260918-1918-133a739f";
import { getContractGrantedClaimIds, getPlotRestriction } from "./systems/operatingRights.js?v=fresh-20260918-1918-133a739f";
import { getNearbyWorldSite, getNearestWorldSite, isInSiteRange } from "./systems/worldSites.js?v=fresh-20260918-1918-133a739f";
import { createGameState } from "./state/gameState.js?v=fresh-20260918-1918-133a739f";
import { canSpendCredits, debitCredits, depositCredits, getCredits, spendCredits } from "./systems/accounts.js?v=fresh-20260918-1918-133a739f";
import { PLAYER_ATTRIBUTED_CAUSES } from "./systems/eventLedger.js?v=fresh-20260918-1918-133a739f";
import { getResourceColor, getResourceShape, resourceTypesMatch } from "./systems/resourceDefinitions.js?v=fresh-20260918-1918-133a739f";
import { terminateDestroyedActor } from "./systems/actorLifecycle.js?v=fresh-20260918-1918-133a739f";
