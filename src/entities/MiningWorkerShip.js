import { advanceFlightBody, getTurnTowardAngle, wrapAngle } from "../systems/flightPhysics.js?v=fresh-20260726-0149-ac3c0eb";
import { normalizeResourceType } from "../systems/resourceDefinitions.js?v=fresh-20260726-0149-ac3c0eb";
...
export class MiningWorkerShip {
  ...
  assign({ contractId, resourceId, quantity, destination }) {
    if (this.assignment || quantity <= 0) return false;
    this.assignment = { contractId, resourceId: normalizeResourceType(resourceId), quantity, destination };
  ...
  update(deltaSeconds, world) {
    ...
    const pickup = nearest(world.pickups.filter((item) => normalizeResourceType(item.type) === this.assignment.resourceId), this.position);
