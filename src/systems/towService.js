import { buildPhysicalTransportationRoute, createTransportationNetwork, findTransportationRoute } from "./transportationPlanning.js?v=fresh-20260822-0010-4a2fda0d";
...
import { resolveInstitutionPolicy } from "./institutionDecision.js?v=fresh-20260822-0010-4a2fda0d";
...
import { getServiceCost, recordServiceCost } from "./costBasis.js?v=fresh-20260822-0010-4a2fda0d";
import { getRelationshipProjection, recordDeliveryOutcome } from "./relationshipProjections.js?v=fresh-20260822-0010-4a2fda0d";
import { authorizeWreckSalvage, completeWreckSalvage } from "./wreckRegistry.js?v=fresh-20260822-0010-4a2fda0d";
import { applyCraftUse, ensureCraftComponents, getWorstComponent } from "./componentCondition.js?v=fresh-20260822-0010-4a2fda0d";
import { DIAGNOSTIC_STATE, recordDiagnostic } from "./diagnostics.js?v=fresh-20260822-0010-4a2fda0d";
...
export function createTowServiceManager({ state, ships = [], destinations = [], now = () => Date.now(), onWreckRecovered = () => {} }) {
  ...
  function claimNextSalvageJob() {
    ...
    if (!wreck || wreck.status !== "sprc-owned-awaiting-recovery") return null;
  ...
  function advanceSalvageWork() {
    Object.values(towing.requests).filter((request) => request.status === "recovering-wreck" && now() >= request.completesAt).forEach((request) => {
      ...
      state.ledger.recordEvent("wreck.salvageDelivered", { institutionId: state.sprc.institution.id, previousOwnerInstitutionId: completed.previousOwnerInstitutionId, actorInstitutionId: towing.controller.id, vehicleId: towing.vehicle.id, requestId: request.id, contractId: contract.id, wreckId: completed.id, destinationSiteId: request.destinationSiteId, salvagerId: towing.institution.id, fee: request.fee, plannedSalvageYield: completed.plannedSalvageYield }, { visible: true, message: `${towing.controller.name} delivered ${completed.shipName} to Sal; First Reach Recovery earned ${request.fee} cr and The Maw can now dismantle the wreck.` });
    });
  }
