import { createNeedRecord, planResponses, resolveInstitutionPolicy } from "./institutionDecision.js?v=fresh-20260822-1330-factories";
...
export function deriveFleetNeeds({ fleet, policy, now, makeId = (kind, id) => `need:${kind}:${id ?? "fleet"}` }) {
  ...
  // sold, and there is no cap on the patience it buys. An operator is allowed to
  // be wrong, strand its own hub, and leave a derelict behind — that is a
  // decision the world can show the player, not an invariant to protect.
  const patienceFor = (ship) => {
    ...
    // No ceiling on this. The number is real credits of real open work, so it
    // bounds itself — and an operator holding a hull because the work is worth
    // ten hulls is not being protected, it is being right.
    return base * (1 + (worth / policy.hireCost) * (policy.capabilityPatience ?? 1));
  };
